...
class AcasEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        self.interest_time=100
        self.action_choices=[0,1,2,3,4]
        self.proba_actions=[0.8,0.5,0.5,0.5,0.5]
        self.last_a = 0
    ...
    def step(self, action):
        ...
        time_turn_intruder = random.randrange(60, 70)
        random_action_intruder= random.choices(self.action_choices, weights=self.proba_actions, k=1)[0]
        own.head = rad_mod(own.head + self.act_to_angle[int(action)])
        if time_turn_intruder==self.current_time_step:
            intruder.head= rad_mod(intruder.head+self.act_to_angle[random_action_intruder])
