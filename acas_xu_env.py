...
# TODO supprimer ou modifier ? Comprendre ce que ça fait ( CHATGPT ?)
class Encounter:
...
class AcasEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        self.action_space = spaces.Discrete(5) #ACTION DISCRETIZATION : 0,1,2,3,4 (COC, WR, WL, R, L)


        self.observation_space = spaces.Box(
            low = -np.inf,
            high = np.inf,
            shape=(5,), # la taille du vecteur qu'on observe, rho, theta, psi, v_own, v_int
            dtype=np.float32
        )
    ...
    def step(self, action):
        ...
        terminated = np.array_equal(self.own_location, self.target_location) # TODO 
        truncated = np.array_equal()#  TODO time_steps = max  OU si rho < epsilon 
